
export default function Typography(){
    return(
        <>
        <div id="content" className="site-content">
        <div className="page-header flex items-center justify-center py-8 bg-gray-100">
                <div className="container mx-auto px-2">
                    <div className="inner flex items-center justify-between">
                        <h1 className="page-title text-5xl font-bold">Typography</h1>
                        <ul id="breadcrumbs" className="breadcrumbs flex space-x-2">
                            <li><a href="index.html" className="text-blue-500 hover:underline">Home</a></li>
                            <li className="text-gray-500">Typography</li>
                        </ul>    
                    </div>
                </div>
            </div>
        <section className="py-10">
  <div className="container mx-auto">
    <div className="row">
      <div className="col-span-12">
        <h1 className="text-5xl font-extrabold mb-4">Heading 1</h1>
        <p className="mb-4">
          SEO gets a bad reputation as volatile and full of guesswork. Results seem unattainable and the entire process is daunting. Onum takes the unknown out of SEO with a transparent methodology that effectively improves your rankings so you know what we’re doing and why we’re doing it every step of the way. Onum doesn’t have a “secret recipe;” we use real people to evaluate real results and test best practices to earn first page rankings and organic traffic.
        </p>
        <h2 className="text-3xl font-extrabold mb-4">Heading 2</h2>
        <p className="mb-4">
          SEO gets a bad reputation as volatile and full of guesswork. Results seem unattainable and the entire process is daunting. Onum takes the unknown out of SEO with a transparent methodology that effectively improves your rankings so you know what we’re doing and why we’re doing it every step of the way. Onum doesn’t have a “secret recipe;” we use real people to evaluate real results and test best practices to earn first page rankings and organic traffic.
        </p>
        <h3 className="text-2xl font-extrabold mb-4">Heading 3</h3>
        <p className="mb-4">
          SEO gets a bad reputation as volatile and full of guesswork. Results seem unattainable and the entire process is daunting. Onum takes the unknown out of SEO with a transparent methodology that effectively improves your rankings so you know what we’re doing and why we’re doing it every step of the way. Onum doesn’t have a “secret recipe;” we use real people to evaluate real results and test best practices to earn first page rankings and organic traffic.
        </p>
        <h4 className="text-xl font-extrabold mb-4">Heading 4</h4>
        <p className="mb-4">
          SEO gets a bad reputation as volatile and full of guesswork. Results seem unattainable and the entire process is daunting. Onum takes the unknown out of SEO with a transparent methodology that effectively improves your rankings so you know what we’re doing and why we’re doing it every step of the way. Onum doesn’t have a “secret recipe;” we use real people to evaluate real results and test best practices to earn first page rankings and organic traffic.
        </p>
        <h5 className="text-lg font-extrabold mb-4">Heading 5</h5>
        <p className="mb-4">
          SEO gets a bad reputation as volatile and full of guesswork. Results seem unattainable and the entire process is daunting. Onum takes the unknown out of SEO with a transparent methodology that effectively improves your rankings so you know what we’re doing and why we’re doing it every step of the way. Onum doesn’t have a “secret recipe;” we use real people to evaluate real results and test best practices to earn first page rankings and organic traffic.
        </p>
        <h6 className="text-base font-extrabold mb-4">Heading 6</h6>
        <p className="mb-4">
          SEO gets a bad reputation as volatile and full of guesswork. Results seem unattainable and the entire process is daunting. Onum takes the unknown out of SEO with a transparent methodology that effectively improves your rankings so you know what we’re doing and why we’re doing it every step of the way. Onum doesn’t have a “secret recipe;” we use real people to evaluate real results and test best practices to earn first page rankings and organic traffic.
        </p>
      </div>
    </div>
  </div>
</section>
</div>
        </>
    );
};